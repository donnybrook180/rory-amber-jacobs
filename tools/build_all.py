...
def main():
    ...
    for lang in [] if args.index_only else cfg["languages"]:
        out = ROOT / lang["out"]
        # No `source` = not built from Excel. That language's committed
        # <lang>/words.*.json is the source of truth (generated by
        # gen_lang_content.py); its languages.json entry exists only so
        # gen_audio.py can find the voice. Skip — the discovery pass below still
        # publishes it via its committed manifest. This is also the Phase-0
        # guard: the placeholder de/es masters were quarantined to
        # resources/_fake/, and dropping their `source` here means no
        # re-import can clobber the real list. Do NOT re-add a `source`.
        if "source" not in lang:
            print(f"\n=== skipping {lang['targetLang']}: no source "
                  f"(committed JSON is source of truth) ===")
            continue
        source = ROOT / lang["source"]
        # A language can be registered before its master spreadsheet is dropped in.
        # Skip — don't fail the build — and leave it out of index.json until the
        # master exists.
